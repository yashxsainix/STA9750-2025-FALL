...
if (offline) {
  ...
  get_acs_cbsa <- function(y){
    tidycensus::get_acs(geography = "metropolitan statistical area/micropolitan statistical area",
                        ...
                        geometry = FALSE) |>
      janitor::clean_names() |>
      transmute(cbsa = geoid, name = name, year = y,
  ...
  # Simple placeholder permits (replace with class permits if available)
  permits_cbsa <- acs_cbsa |>
    transmute(cbsa, year, permits = pmax(50, round(0.002 * pop + runif(n(), -500, 5000))))
}
...
**Definition.** \( \text{RBI}_{i,t} = 100 \times \frac{(rent/income)_{i,t}}{\overline{(rent/income)}_{i,baseline}} \)
...
p <- ggplot(cbsa_panel, aes(year, income, group=cbsa, color=cbsa %in% hi)) +
  geom_line(alpha=.55) +
...
kpi <- tibble(
  ...
  Value = c(round(median(rel$rbi, na.rm=TRUE),1),
            (hgc |> arrange(desc(hgc)) |> slice(1) |> dplyr::pull(cbsa)) %||% "N/A",
